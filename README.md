...
`HF_TOKEN` is optional — without it, everything works except the "Generate 3D preview" buttons, which fail with a clear error instead of crashing. Get a free token at [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) (a free account, no card, a read-level token is enough). Note the Space needs to fetch the dish photo over the public internet, so generation only works once an item (with its photo) is reachable at a real public URL — it won't work against `localhost` unless you tunnel it (e.g. ngrok). Since this runs on free community demo Spaces rather than a paid API, expect it to occasionally be slow or briefly unavailable.

`HF_TOKEN_2`, `HF_TOKEN_3`, ... (also optional) add tokens from additional Hugging Face accounts — since each account gets its own separate daily ZeroGPU quota, the server automatically rotates to the next token when one account's TRELLIS quota runs out, multiplying the effective daily "high quality" generation budget.
