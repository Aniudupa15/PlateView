...
- **AI-generated 3D previews** — from a dish's edit screen, "Generate 3D preview from photo" sends the uploaded photo to [TripoSR](https://github.com/VAST-AI-Research/TripoSR) (open-source, MIT-licensed single-image 3D reconstruction) hosted on [Replicate](https://replicate.com), and stores the resulting `.glb` back on the item — no manual 3D modeling required.
...
| AI 3D generation | [TripoSR](https://github.com/VAST-AI-Research/TripoSR) via [Replicate](https://replicate.com), with an `obj2gltf` fallback conversion if the output isn't already `.glb` |
| Linting | Oxlint |
...
cd server
npm install
cp .env.example .env   # fill in DATABASE_URL, JWT_SECRET, ADMIN_EMAIL, ADMIN_PASSWORD, REPLICATE_API_TOKEN
...
`REPLICATE_API_TOKEN` is optional — without it, everything works except the "Generate 3D preview" button, which fails with a clear error instead of crashing. Get a free token at [replicate.com](https://replicate.com/account/api-tokens); the free tier covers a meaningful number of generations before billing kicks in. Note that Replicate needs to fetch the dish photo over the public internet, so generation only works once an item (with its photo) is reachable at a real public URL — it won't work against `localhost` unless you tunnel it (e.g. ngrok).
...
└── server/                # Express + Prisma API
    ...
    └── src/
        ...
        ├── replicate.ts        # Calls TripoSR on Replicate, converts OBJ output to .glb if needed
